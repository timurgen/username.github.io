...
package body Main.Dispatchers is
         ...
         return Response.Build
           (MIME.Text_HTML,
            String'(Templates.Parse
              (Filename     => To_String (Web_Root) & "public_html/index.html",
               Translations => Translations)));

      else
         return Response.Acknowledge (Messages.S404, "Page not found");
      end if;
   end Dispatch;
   ...
   is
      pragma Unreferenced (Dispatcher);
      URI  : constant String := Status.URI (Request);
      File : constant String := To_String (Web_Root) & URI (URI'First + 1 .. URI'Last);
   begin
